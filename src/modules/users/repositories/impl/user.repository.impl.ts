...
export class UserRepositoryImpl implements IUserRepository {
    ...
    async update(id: string, data: UpdateUserDto): Promise<UserWithProfile | undefined> {
        try {
            // neon-http does not support db.transaction(); run queries sequentially.
            // For CF Workers, neon-http is required (stateless HTTP, no shared I/O).
    ...
    async delete(id: string): Promise<User | undefined> {
        try {
            // db.batch() sends both deletes atomically in a single HTTP round-trip.
            const [, deletedUsers] = await db.batch([
                db.delete(profiles).where(eq(profiles.userId, id)),
                db.delete(users).where(eq(users.id, id)).returning(),
            ]);

            const [deletedUser] = deletedUsers as (typeof users.$inferSelect)[];
