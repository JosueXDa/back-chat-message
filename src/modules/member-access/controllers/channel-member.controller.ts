...
import { toHTTPException } from "../errors/access.errors";
...
export class ChannelMemberController {
    ...
    private registerRoutes() {
        this.router.get("/joined", requireAuth, async (c) => {
            try {
                const user = c.get("user");
                const channels = await this.channelMemberService.getChannelsByUserId(user.id);
         ...
         */
        this.router.get("/:channelId", requireAuth, async (c) => {
            try {
                const user = c.get("user");
         ...
         */
        this.router.get("/:channelId/role/:userId", requireAuth, async (c) => {
            try {
                const channelId = c.req.param("channelId");
         ...
         * }
         */
        this.router.patch("/:channelId/:userId/role", requireAuth, async (c) => {
            try {
                const user = c.get("user");
                const channelId = c.req.param("channelId");
                const userId = c.req.param("userId");
         ...
         */
        this.router.delete("/:channelId/:userId", requireAuth, async (c) => {
            try {
                const user = c.get("user");
                const channelId = c.req.param("channelId");
                const userId = c.req.param("userId");
         ...
         */
        this.router.get("/is-joined/:channelId", requireAuth, async (c) => {
            try {
                const user = c.get("user");
                const channelId = c.req.param("channelId");
                const isJoined = await this.channelMemberService.isJoined(channelId, user.id);
